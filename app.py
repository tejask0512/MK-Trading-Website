...
# Database migration utility route (protected, only for admins)
@app.route("/admin/migrate-db")
@login_required
def migrate_db():
    """Migrate the database schema (only for development)."""
    if session.get("user_id") != 1:  # Assuming admin has ID 1
